...
/*virtual*/void CCziMetadataDocumentInfo::EnumDimensions(const std::function<bool(libCZI::DimensionIndex)>& enumDimensions)
{
...
void CCziMetadataDocumentInfo::ParseDimensionInfo()
{
    ...
    if (!np.empty())
    {
        for (int i = 0; i < sizeof(DimAndNodeNames) / sizeof(DimAndNodeNames[0]); ++i)
        {
            ...
            {
                bool startExplicitlyStated = false;
                bool endExplicitlyStated = false;
                int start = 0, size = 1;
                if (!nStart.empty())
                {
                    start = nStart.text().as_int();
                    startExplicitlyStated = true;
                }

                if (!nSize.empty())
                {
                    size = nSize.text().as_int();
                    endExplicitlyStated = true;
                }

                this->dimensions[DimAndNodeNames[i].index] = make_shared<CCziDimensionInfo>(
                    DimensionAndStartSize{ DimAndNodeNames[i].index ,start,size },
