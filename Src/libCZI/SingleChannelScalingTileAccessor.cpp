...
/// Create a vector with indices (into the specified vector with SubBlock-infos) so that the
/// indices give the items sorted by their "zoom"-factor. (A zoom of "1" means that the
/// subblock is on layer-0). Subblocks of a higher pyramid-layer are at the end of the list.
///
/// \param  sbBlks  The vector of subblock-infos for which to create the sorted indices.
/// \param  sortByM Whether to sort the subblocks by their zoom level AND the M-Index or only by
///                 zoom level.
///
/// \returns   A vector with indices which give the subblocks sorted by their zoom (biggest zoom
///            first).
...
void CSingleChannelScalingTileAccessor::Paint(libCZI::IBitmapData* bmDest, const libCZI::IntRect& roi, const SubSetSortedByZoom& sbSetSortedByZoom, float zoom, const libCZI::ISingleChannelScalingTileAccessor::Options& options)
{
    ...
    const int idxOf1stSubBlockOfZoomGreater = CSingleChannelScalingTileAccessor::GetIdxOf1stSubBlockWithZoomGreater(sbSetSortedByZoom.subBlocks, sbSetSortedByZoom.sortedByZoom, zoom / 1.05f);
...
/// Using the specified ROI, determine the scenes it intersects with. If the subblock-
/// repository does not contain an "S-dimension" we return an empty result.
///
/// \param  roi             The ROI.
/// \param  pSceneIndexSet  Set of the scenes which are "allowed". May be null, in which case all
...
/// Gets the subset of subblocks intersecting with the ROI, having the specified plane-
/// coordinate and where their scene-index is among the ones given. If the subblock has no
/// scene-index, the filtering by-scene-index is not applied.
///
/// \param  roi             The region-of-interest rectangle.
/// \param  planeCoordinate The plane coordinate.
/// \param  allowedScenes   The list of allowed scenes.
/// \param  sortByM         Whether to sort the subblocks by their zoom level AND the M-Index or
///                         only by zoom level.
///
