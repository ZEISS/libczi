...
TEST(MetadataReading, DimensionInfo2Test)
{
    ...
    bool startExplicitlyStated, endExplicitlyStated;
    dimInfo->GetIntervalIsExplicitlyStated(&startExplicitlyStated, &endExplicitlyStated);
    EXPECT_FALSE(startExplicitlyStated) << "startExplicitlyStated expected to be 'false'";
    EXPECT_TRUE(endExplicitlyStated) << "sizeExplicitlyStated expected to be 'true'";
...
TEST(MetadataReading, DimensionInfoChannels2Test)
{
    ...
    wstring channelName, channelId;
    ASSERT_TRUE(ch1->TryGetAttributeName(&channelName) && channelName == L"Ch1");
    ASSERT_TRUE(ch1->TryGetAttributeId(&channelId) && channelId == L"Ch1");
    ...
    ASSERT_TRUE(ch2->TryGetAttributeName(&channelName) && channelName == L"ch2");
    ASSERT_TRUE(ch2->TryGetAttributeId(&channelId) && channelId == L"FITC");
    b = ch2->TryGetIlluminationWavelength(&characteristics);
    EXPECT_FALSE(b);

    auto ch3 = channels->GetChannel(2);
    ASSERT_TRUE(ch3->TryGetAttributeName(&channelName) && channelName == L"ch3");
    ASSERT_TRUE(ch3->TryGetAttributeId(&channelId) && channelId == L"Rhodamin");
